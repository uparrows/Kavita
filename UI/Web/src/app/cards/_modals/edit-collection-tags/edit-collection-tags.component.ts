...
export class EditCollectionTagsComponent implements OnInit {
  ...
  async save() {
    ...
    if (unselectedIds.length == this.series.length && !await this.confirmSerivce.confirm('警告！ 未选择系列，保存将删除标签。 你确定你要继续吗?')) {
      return;
    }
