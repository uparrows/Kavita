...
export class ErrorInterceptor implements HttpInterceptor {
      ...
      catchError(error => {
        ...
        switch (error.status) {
          ...
          default:
            ...
            if (this.toastr.previousToastMessage !== '意料之外的错误。') {
              this.toastr.error('意料之外的错误。');
            }
  ...
  private handleValidationError(error: any) {
    ...
    if (Array.isArray(error.error)) {
      ...
      if (error.statusText === '错误的请求') {
        this.toastr.error(error.error, error.status);
      } else {
        this.toastr.error(error.statusText === '好的' ? error.error : error.statusText, error.status);
      }
    }
  }

  private handleNotFound(error: any) {
    this.toastr.error('该网址不存在。'); 
  }

  private handleServerException(error: any) {
    const err = error.error;
    if (err.hasOwnProperty('message') && err.message.trim() !== '') {
      if (err.message != '用户未通过身份验证') {
        console.log('500 error: ', error);
      }
      this.toastr.error(err.message);
    } else {
      this.toastr.error('存在未知的严重错误。');
  ...
  private handleAuthError(error: any) {
    ...
    this.accountService.currentUser$.pipe(take(1)).subscribe(user => {
      if (user) {
        this.toastr.error(error.statusText === '好的' ? '未经授权' : error.statusText, error.status);
      }
