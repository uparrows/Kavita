...
export class InfiniteScrollerComponent implements OnInit, OnChanges, OnDestroy {
  ...
  handleScrollEvent(event?: any) {
    ...
    if (this.isScrolling && this.currentPageElem != null && this.isElementVisible(this.currentPageElem)) {
      this.debugLog('[Scroll] 图像从滚动中可见，isScrolling 现在为 false');
  ...
  initWebtoonReader() {
    ...
    this.debugLog('[INIT] 预取页面 ' + startingIndex + ' 到 ' + endingIndex + '. 当前页面: ', this.pageNum);
  ...
  onImageLoad(event: any) {
    ...
    this.debugLog('[Image Load] 图像加载: ', imagePage);
    ...
    if (imagePage === this.pageNum) {
      Promise.all(Array.from(document.querySelectorAll('img'))
        ...
        .then(() => {
          this.debugLog('[Image Load] ! 加载的当前页面 !', this.pageNum);
  ...
  handleIntersection(entries: IntersectionObserverEntry[]) {
    if (!this.allImagesLoaded || this.isScrolling) {
      this.debugLog('[Intersection] 图像未加载（或执行滚动操作），跳过任何滚动计算');
      return;
    }

    entries.forEach(entry => {
      const imagePage = parseInt(entry.target.attributes.getNamedItem('page')?.value + '', 10);
      this.debugLog('[Intersection] 页 ' + imagePage + ' 可见: ', entry.isIntersecting);
      if (entry.isIntersecting) {
        this.debugLog('[Intersection] ! 页 ' + imagePage + ' 刚进入画面');
  ...
  setPageNum(pageNum: number, scrollToPage: boolean = false) {
    ...
    if (scrollToPage) {
      ...
      if (currentImage !== null && !this.isElementVisible(currentImage)) {
        this.debugLog('[GoToPage] 滚动到页面', this.pageNum);
        this.scrollToCurrentPage();
      }
    }
  }
  ...
  scrollToCurrentPage() {
    ...
    setTimeout(() => {
      if (this.currentPageElem) {
        this.debugLog('[Scroll] 滚动到页面 ', this.pageNum);
  ...
  loadWebtoonImage(page: number) {
    ...
    if (this.imagesLoaded.hasOwnProperty(page)) {
      this.debugLog('\t[PREFETCH] 跳过预取 ', page);
      return;
    }
    this.debugLog('\t[PREFETCH] 预取 ', page);
  ...
  attachIntersectionObserverElem(elem: HTMLImageElement) {
    if (elem !== null) {
      ...
      this.debugLog('Attached Intersection 附加到页面', this.readerService.imageUrlToPageNum(elem.src));
    } else {
      console.error('无法在元素上附加观察'); // This never happens
    }
  }
  ...
  prefetchWebtoonImages() {
    ...
    this.debugLog('\t[PREFETCH] 预取页面: ' + startingIndex + ' to ' + endingIndex);
    for(let i = startingIndex; i < endingIndex; i++) {
      this.loadWebtoonImage(i);
    }
