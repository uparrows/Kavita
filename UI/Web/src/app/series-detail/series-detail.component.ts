...
export class SeriesDetailComponent implements OnInit {
  ...
  async deleteSeries(series: Series) {
    if (!await this.confirmService.confirm('确定要删除此系列吗？ 它不会修改磁盘上的文件.')) {
      this.actionInProgress = false;
      return;
    }
  ...
  loadSeries(seriesId: number) {
    ...
    this.seriesService.getSeries(seriesId).subscribe(series => {
      ...
      this.titleService.setTitle('Kavita - ' + this.series.name + ' 详细');
  ...
  openChapter(chapter: Chapter) {
    if (chapter.pages === 0) {
      this.toastr.error('没有页面。 Kavita 无法读取此存档.');
      return;
    }
  ...
  async promptToReview() {
    ...
    config.header = '确认';
    config.content = '你想写评论吗?';
    config.buttons.push({text: '不想', type: 'secondary'});
    config.buttons.push({text: '想', type: 'primary'});
    if (shouldPrompt && await this.confirmService.confirm('你想写评论吗?', config)) {
