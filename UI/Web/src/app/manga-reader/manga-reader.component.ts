...
export class MangaReaderComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.accountService.currentUser$.pipe(take(1)).subscribe(user => {
      if (user) {
        ...
        this.memberService.hasReadingProgress(this.libraryId).pipe(take(1)).subscribe(progress => {
          if (!progress) {
            this.toggleMenu();
            this.toastr.info('随时点击图像以打开菜单。 您可以配置不同的设置或通过单击进度条进入页面。 点击图像的边移到下一页/上一页.');
          }
        });
      } else {
  ...
  updateTitle(chapterInfo: ChapterInfo) {
      ...
      if (chapterInfo.isSpecial && chapterInfo.volumeNumber === '0') {
        ...
        this.subtitle = '章 ' + chapterInfo.chapterNumber;
      } else {
        this.subtitle = '卷 ' + chapterInfo.volumeNumber;
  ...
  loadChapter(chapterId: number, direction: 'next' | 'prev') {
    if (chapterId >= 0) {
      ...
      this.toastr.warning('找不到 ' + direction + ' 章');
      this.isLoading = false;
      if (direction === 'prev') {
